<?php
	...
	function nieuws_normal_view(){
		...
		$content = content_query($type = content_get_type("Nieuwsitem"), "loop");
...
<?php
	...
	function nieuws_create_new(){
		...
		<div class="col-sm-4 left-align">
			...
			<input type="submit" style="float: right; margin-left: 5px;" value="Publish" class="btn"> <input type="submit" value="Save as draft" style="float: right;"class="btn">
		</div>
		</form>
	...
	function nieuws_edit(){
		...
		$content = content_query($type = content_get_type("Nieuws"), "static", $post);
	...
	function nieuws_insert(){
		...
		content_add($title, $content, $user, 1, content_get_type("Nieuws"));
